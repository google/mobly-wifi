...
from mobly.controllers.wifi.lib.encryption import open as encryption_open
...
class SimpleConnectTest(base_test.BaseTestClass):
  ...
  def setup_test(self):
    forget_all_wifi(self.ad)
    disable_wifi(self.ad)
...
###################Utilities for controlling Android WiFi###################
...
def connect_to_wifi(
  ...
  ad.adb.shell(cmd)
  if not with_internet_check:
    if not _wait_for_ssid(ad, ssid):
